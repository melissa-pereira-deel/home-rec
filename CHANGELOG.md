...
This release also carries every fix prepared as 1.1.2, which was never published. **If you are updating from 1.1.1, all six fixes below are new to you** — including a crash that could take a long recording with it.
...
- **Quitting while recording now finishes the recording first** — Pressing ⌘Q, or choosing Quit from any menu, ended Home Rec immediately with the audio file still open. The take was cut off rather than closed: a WAV that under-reported its own length, an M4A missing the index that makes it playable, and a FLAC that would not open at all until you ran Recover Recordings on it. Home Rec now finishes writing the file before it quits — normally fast enough that you will not notice — and only then closes. Recover Recordings is for power cuts and crashes; it should never have been what caught you pressing Quit. If finishing somehow cannot complete, Home Rec still quits after a few seconds rather than refusing to close, and leaves the file in a state Recover Recordings can repair. (BL-174)
- **A recording that fails to start no longer leaves a stray file behind** — If starting a recording failed part-way — the chosen app quitting, Screen Recording permission being withdrawn, the capture system refusing to start — Home Rec left an empty file sitting in your save folder alongside your real recordings, and kept the audio file open internally. Worse, Recover Recordings would then list that empty file and tell you a recording had been interrupted, when nothing had ever been recorded. Home Rec now cleans up everything it opened when a start fails, removes the empty file, and reports the actual reason it couldn't start. Recover Recordings no longer offers files from takes that never began. (BL-171a)
